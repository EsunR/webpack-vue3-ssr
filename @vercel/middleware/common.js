...
function commonMiddleware(app) {
    ...
    const vercelPath = path.resolve(__dirname, '../client');
    // 列出 vercelPath 目录下的所有文件
    console.log(fs.readdirSync(vercelPath));
    console.log(fs.readdirSync(path.resolve(vercelPath, './static')));
    app.use(
        express_1.default.static(process.env.VERCEL ? vercelPath : 'client', {
            index: false,
        })
    );
